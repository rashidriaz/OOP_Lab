...
class Date {
    ...
    public Date(int month, int day, int year) {
        ...
        if (year < 100) {
            ...
            addPreFixToYear(day, month, year);
        } else {
            this.year = year;
        }
        this.month = month;
    ...
    private void addPreFixToYear(int day, int month, int year) {
        LocalDateTime currentDate = LocalDateTime.now();
        if (year < 100 && (year + 2000) > currentDate.getYear()) {
            this.year = 1900 + year;
        } else if (year <= (currentDate.getYear() - 2000)) {
            int temporary = 2000 + year;
            if (dateIsGreaterThanCurrentDate(day, month, temporary)) {
                this.year = 1900 + year;
            }else{
                this.year = temporary;
            }
